...
const AddNote = ({ notesRef }: any) => {
  ...
  const handleClickOutside = (e: Event) => {
    const { current: wrap }: any = titleRef;
    const { current: wrap2 }: any = pinRef;

    if (
      wrap &&
      !wrap.contains(e.target) &&
      wrap2 &&
      !wrap2.contains(e.target)
    ) {
      setShowInput(false);
    }
  };
